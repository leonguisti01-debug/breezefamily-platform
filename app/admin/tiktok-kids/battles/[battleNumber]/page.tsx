...
export default function BattleJudgingPage() {
  ...
  const battleNumber = Number(params.battleNumber);
  ...
  async function checkAccess() {
    ...
    if (
      ...
      alert("Only Super Admins can judge battles.");
  ...
  async function loadBattle() {
    ...
    if (contestantsError) {
      console.error(contestantsError);
      alert(contestantsError.message);
      return;
    }

    const left = contestants?.find(
      (c) => c.id === battleRow.contestant_left_id
    );

    const right = contestants?.find(
      (c) => c.id === battleRow.contestant_right_id
    );

    if (!left || !right) {
      alert("One of the contestants could not be found.");
      return;
    }

    const loadedBattle: Battle = {
      id: battleRow.id,
      battle_number: battleRow.battle_number,
      contestant_left: left,
      contestant_right: right,
      winner_id: battleRow.winner_id,
      loser_id: battleRow.loser_id,
      status: battleRow.status,
      judged_by: battleRow.judged_by,
      judged_at: battleRow.judged_at,
    };

    setBattle(loadedBattle);

    if (battleRow.winner_id) {
      setSelectedWinner(battleRow.winner_id);
    }
  }
  ...
  async function saveResult() {
    ...
    if (!selectedWinner) {
      alert("Please select who goes through.");
      return;
    }
    ...
    try {
      ...
      if (nextBattle <= 15) {
        router.push(
          `/admin/tiktok-kids/battles/${nextBattle}`
        );
      } else {
        router.push("/admin/tiktok-kids/battles");
      }
  ...
  const left = battle.contestant_left;
  const right = battle.contestant_right;
  const completed = battle.status === "completed";

  const winner =
    selectedWinner === left.id
      ? left
      : selectedWinner === right.id
      ? right
      : null;
  ...
  return (
    <main className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-[1600px] mx-auto px-5 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-zinc-900 border border-white/10 px-4 py-3 rounded-xl font-black uppercase text-sm"
            >
              Battles
            </button>
            ...
            <button
              onClick={logout}
              className="bg-red-600 px-4 py-3 rounded-xl font-black uppercase text-sm"
            >
              Logout
            </button>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-6 py-8">
        ...
        {completed && (
          <div className="mb-6 bg-green-500/10 border border-green-500/30 rounded-2xl p-5 text-center">

            <p className="text-green-400 font-black uppercase">
              Battle Completed
            </p>
        ...
        <div className="mt-8 max-w-4xl mx-auto">
          ...
          <div className="border border-white/10 bg-zinc-950 rounded-3xl p-6 md:p-8">

            <p className="text-center text-white/40 text-xs font-black uppercase tracking-[4px]">
              Who goes through?
            </p>
      ...
      {showConfirmation &&
        winner &&
        loser && (
          <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-md flex items-center justify-center p-5">

            <div className="w-full max-w-xl bg-zinc-950 border border-white/10 rounded-[30px] p-7 md:p-10 shadow-2xl">

              <p className="text-cyan-400 text-xs font-black uppercase tracking-[4px] text-center">
                Confirm Result
              </p>

              <h2 className="text-3xl md:text-4xl font-black uppercase italic text-center mt-3">
                Are You Sure?
              </h2>
              ...
              <div className="mt-8 bg-green-500/10 border border-green-500/30 rounded-2xl p-5 text-center">

                <p className="text-green-400 text-xs font-black uppercase tracking-[3px]">
                  Goes Through
                </p>

                <p className="text-2xl md:text-3xl font-black uppercase mt-2">
                  {winner.full_name}
                </p>
              ...
              <div className="mt-3 bg-red-500/10 border border-red-500/30 rounded-2xl p-5 text-center">

                <p className="text-red-400 text-xs font-black uppercase tracking-[3px]">
                  Eliminated
                </p>
    ...
    >
      ...
      <div className="relative">
        ...
        {contestant.photo_url ? (
          ...
          <div className="w-full h-[360px] md:h-[470px] bg-zinc-900 flex items-center justify-center text-white/20 font-black uppercase">
            No Photo
          </div>
        )}

        {selected && (
          <div className="absolute top-5 left-5 bg-green-400 text-black px-4 py-2 rounded-full font-black uppercase text-sm">
            THROUGH
          </div>
        )}
      ...
      <div className="p-6">
        ...
        <p className="text-cyan-400 text-xs font-black uppercase tracking-[3px]">
          {right ? "Contestant 2" : "Contestant 1"}
        </p>

        <h2 className="text-3xl md:text-4xl font-black uppercase leading-tight mt-1">
        ...
        <div className="mt-6 space-y-4">
          ...
          <InfoRow
            ...
            value={contestant.tiktok_username}
            highlight
          />
          ...
          <InfoRow
            ...
            value={contestant.talent_category}
          />

          <InfoRow
            label="Mentor"
            value={contestant.mentor}
          />
          ...
          <InfoRow
            ...
            value={contestant.performance_date}
          />
        ...
        {contestant.audition_notes && (
          <div className="mt-6 pt-6 border-t border-white/10">
            ...
            <p className="text-white/40 text-xs uppercase font-black tracking-wider">
              Audition Notes
            </p>
...
function InfoRow({
  ...
  return (
    <div>
      ...
      <p className="text-white/35 text-xs uppercase font-black tracking-wider">
        {label}
      </p>
